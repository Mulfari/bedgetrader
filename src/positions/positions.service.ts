...
import { BybitClosedPositionResponse } from './closed-position.interface';
...
export class PositionsService {
  ...
  /**
   * Obtiene las operaciones cerradas de una subcuenta de Bybit en un intervalo de tiempo
   * @param subaccount Subcuenta de la que se quieren obtener las operaciones cerradas
   ...
   */
  async getBybitClosedPositions(subaccount: SubAccount, days: number = 7): Promise<BybitClosedPositionResponse | null> {
    const MAX_RETRIES = 3;
    const RETRY_DELAY = 2000; // 2 segundos entre reintentos
    
    // Limitar a 7 días como máximo (restricción de la API de Bybit)
    const daysToFetch = Math.min(days, 7);
    
    // Calcular timestamps para el intervalo de tiempo
    const endTime = Date.now();
    const startTime = endTime - (daysToFetch * 24 * 60 * 60 * 1000); // daysToFetch días en milisegundos
    
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        this.logger.log(`🔄 Intento ${attempt}/${MAX_RETRIES} de obtener operaciones cerradas para subcuenta ${subaccount.id} (${subaccount.isDemo ? 'DEMO' : 'REAL'})`);
        this.logger.log(`🔹 Intervalo de tiempo: ${new Date(startTime).toISOString()} - ${new Date(endTime).toISOString()} (${daysToFetch} días)`);
        ...
        // QueryString para obtener operaciones cerradas
        const queryParams = { 
          category: "linear",
        ...
        // URL de Bybit para obtener operaciones cerradas
        const baseUrl = subaccount.isDemo 
          ? "https://api-demo.bybit.com"
          : "https://api.bybit.com";
        ...
        // Mostrar las operaciones cerradas en los logs
        const closedPositions = response.data as BybitClosedPositionResponse;
        
        if (closedPositions.result.list.length === 0) {
          this.logger.log(`✅ No hay operaciones cerradas para la subcuenta ${subaccount.id} (${subaccount.isDemo ? 'DEMO' : 'REAL'}) en los últimos ${daysToFetch} días`);
        } else {
          this.logger.log(`✅ Operaciones cerradas para la subcuenta ${subaccount.id} (${subaccount.isDemo ? 'DEMO' : 'REAL'}) en los últimos ${daysToFetch} días:`);
          closedPositions.result.list.forEach((position, index) => {
            this.logger.log(`📊 Operación cerrada ${index + 1}:
              - Símbolo: ${position.symbol}
              - Lado: ${position.side}
              - Tamaño: ${position.qty}
              ...
              - Apalancamiento: ${position.leverage}
              - Fecha de creación: ${new Date(parseInt(position.createdTime)).toISOString()}
              - Fecha de actualización: ${new Date(parseInt(position.updatedTime)).toISOString()}
            `);
          });
        }
        ...
        this.logger.error(`❌ Error en intento ${attempt}/${MAX_RETRIES} para obtener operaciones cerradas:`, {
          message: error.message,
          bybitCode: error.bybitCode,
          bybitMsg: error.bybitMsg,
          status: error.response?.status,
        ...
        // Si es el último intento, devolver null
        if (attempt === MAX_RETRIES) {
          this.logger.error(`❌ No se pudieron obtener las operaciones cerradas después de ${MAX_RETRIES} intentos`);
          return null;
        }
